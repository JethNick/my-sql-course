...
WHERE
	ps.Tariff = (
    SELECT CAST(AVG(ps2.Tariff)As int) FROM PatientStay ps2
   );
...
WHERE
    ptl.PropertyTypeCode not in (select distinct pp.propertytype from PricePaidSW12 pp where pp.street = 'Ranmere Street');
